...
Rectangle {
    ...
    // Hide during first-boot setup (when no users enrolled)
    visible: authState !== stateON && controller.userManager.count > 0
    opacity: (authState !== stateON && controller.userManager.count > 0) ? 1.0 : 0.0
    ...
    function showFailure(attempts) {
        // Don't set failedAttempts here - it's bound to controller.failedAttempts
        // Setting it would break the binding and prevent reset after lockout

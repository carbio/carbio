...
ApplicationWindow {
    ...
    onClosing: function(close) {
        ...
        // Don't call Qt.quit() - let Qt handle natural shutdown
        // Destructor will clean up the thread properly
        close.accepted = true
    }
    ...
    // Service mode shortcut - bypass authentication for emergency enrollment
    Shortcut {
    ...
    Rectangle {
        ...
        height: 50
        color: "#FF9500"
        visible: firstBootSetup.visible
        z: 350  // Above setup wizard
    ...
    Image {
        ...
        Item {
            id: dashboardContent
            anchors.fill: parent
            // PERFORMANCE: layer.enabled removed - was causing 2× rendering overhead (software rendering)
            // layer.enabled: true
            // layer.smooth: true
        ...
        */
        ...
        Image {
            ...
            // Settings button - only visible if user has AdminAccess permission
            Rectangle {
                id: gearButton
                width: 50
                height: 50
                ...
                border.color: "#01E4E0"
                border.width: 2
                visible: controller.authState === 4 && controller.hasPermission(0x0010) // AdminAccess
                opacity: controller.authState === 4 && controller.hasPermission(0x0010) ? 1.0 : 0.0
        ...
        */
        ...
        Rectangle {
            ...
            anchors.left: parent.left
            anchors.margins: 30
            width: Math.max(300, welcomeContent.implicitWidth + 30)
            height: 60
            color: "#1E3E1E"
            radius: 10
            border.color: controller.currentUserRoleColor  // OPTIMIZED: Cached property
            border.width: 2
            visible: controller.authState === 4 && controller.currentUserName !== ""
            opacity: controller.authState === 4 ? 1.0 : 0.0
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        Rectangle {
                            ...
                            height: 8
                            radius: 4
                            color: controller.currentUserRoleColor  // OPTIMIZED: Cached property
                        }

                        Label {
                            text: controller.currentUserRoleName  // OPTIMIZED: Cached property
                            font.pixelSize: 11
                            font.family: "Inter"
                            color: controller.currentUserRoleColor  // OPTIMIZED: Cached property
                        }
                        ...
                        // Admin badge
                        Rectangle {
                            visible: controller.hasPermission(0x0010) // AdminAccess
                            width: 45
                            height: 16
                            radius: 3
                            color: "#FFD700"
        ...
        // First-boot setup wizard (appears when no users enrolled)
        FirstBootSetupWizard {
            id: firstBootSetup
            visible: controller.userManager.count === 0 && controller.templateCount === 0 && controller.sensorAvailable
    ...
    Rectangle {
        ...
        // X mark - OPTIMIZED: Replaced Canvas (CPU-bound) with Rectangles (GPU-accelerated)
        Item {
            anchors.fill: parent
            anchors.margins: 15
            ...
            // First diagonal line (\)
            Rectangle {
                width: Math.sqrt(2 * Math.pow(parent.width, 2))
                height: 3
                color: "#CCCCCC"
                radius: 1.5
                anchors.centerIn: parent

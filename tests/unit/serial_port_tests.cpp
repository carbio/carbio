...
protected:
  ...
  static constexpr std::array<std::uint8_t, 5>    test_data{0x01, 0x02, 0x03, 0x04, 0x05};
  static constexpr std::array<std::uint8_t, 1024> large_buffer{};
  static constexpr serial_config                  default_config{};
};
...
{
  EXPECT_CALL(mock, open("/dev/ttyUSB0", _)).WillOnce(Return());
  mock.open("/dev/ttyUSB0", default_config);
}
...
TEST_F(serial_port_tests, open_with_custom_config)
{
  ...
  EXPECT_CALL(mock, open("/dev/ttyUSB0", testing::_)).WillOnce(Return());
  mock.open("/dev/ttyUSB0", config);
}
...
{
  EXPECT_CALL(mock, write_some(testing::_)).WillOnce(Return(large_buffer.size()));
...
{
  EXPECT_CALL(mock, write_some(testing::_)).WillOnce(Return(test_data.size() / 2));
  std::span<const std::uint8_t> data{test_data};
  auto                          result = mock.write_some(data);
...
TEST_F(serial_port_tests, test_write_exact_with_timeout)
{
  ...
  EXPECT_CALL(mock, write_exact(testing::_, timeout)).WillOnce(Return(test_data.size()));
  std::span<const std::uint8_t> data{test_data};
...
{
  std::vector<std::uint32_t> baud_rates = {9600, 19200, 38400, 57600, 115200, 230400, 460800, 921600};

  for (auto baud : baud_rates)
    {
      serial_config config{.baud_rate = baud,
                                  .data_bits = data_width::_8,
                                  .stop_bits = stop_width::_1,
                                  .parity    = parity_mode::none,
                                  .flow      = flow_control::none};
...
{
  std::vector<data_width> data_bits = {data_width::_5, data_width::_6, data_width::_7, data_width::_8};
...
{
  std::vector<std::chrono::milliseconds> timeouts = {std::chrono::milliseconds{0}, std::chrono::milliseconds{1},
...
{
  EXPECT_CALL(mock, write_some(testing::_)).Times(100).WillRepeatedly(Return(test_data.size()));

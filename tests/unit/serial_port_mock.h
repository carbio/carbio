...
public:
  ...
  MOCK_METHOD(bool, set_data_width, (data_width data), (noexcept));
  MOCK_METHOD(bool, set_stop_width, (stop_width stop), (noexcept));
  MOCK_METHOD(bool, set_parity_mode, (parity_mode parity), (noexcept));
  MOCK_METHOD(bool, set_flow_control, (flow_control flow), (noexcept));
  ...
  MOCK_METHOD(std::size_t, write_exact, (std::span<const std::uint8_t> data, std::chrono::milliseconds timeout));
  MOCK_METHOD(std::size_t, read_exact, (std::span<std::uint8_t> data, std::chrono::milliseconds timeout));
